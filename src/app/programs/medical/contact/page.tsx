...
const HealthFacilitiesPageContent = () => {
  ...
  useEffect(() => {
    const fetchDonations = async () => {
      setIsLoading(true);
      try {
        // Use 'health-facilities' as programId to keep donations separate from other programs
        const donationsResponse = await getProgramDonations('health-facilities', sortOption);
        if (donationsResponse.success && donationsResponse.profile) {
          setDonations(donationsResponse.profile);
        }
        
        const totalResponse = await getProgramTotalDonations('health-facilities');
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      <button
                        ...
                        className="flex-grow md:flex-grow-0 px-6 py-3 text-base font-semibold text-white bg-[#E32613] rounded-lg hover:bg-[#09869a]/90 transition-all"
                      >
                        Donate Now
                      </button>
                      <button 
                    ...
                    {activeTab === 'donations' && (
                      <div>
                        ...
                        <div className="mt-8 p-5 bg-gray-50 rounded-lg border border-gray-200 text-center">
                          <h3 className="text-lg font-medium text-gray-800 mb-2">Support healthcare infrastructure</h3>
                          <p className="text-gray-600 mb-4">Your donation helps improve healthcare facilities in underserved communities</p>
                          <button
                            onClick={handleDonateClick} 
